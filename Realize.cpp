...
namespace warships{
	...
	void Game::processinput(Player player, Player enemy)
	{
		action = getAction();
		switch (action.mode)
		{
	...
	void Player::openCell(Game game, unsigned int x, unsigned int y)
	{
		switch (table_mode[y][x])
		{
		case game.OPEN:
	...
	void Player::openCell(Game game, unsigned int x, unsigned int y, int** ptable_mode, int** ptable_state)
	{
		switch (table_mode[y][x])
		{
		case game.OPEN:
	...
	bool Player::ChekDestroy(Game game, const int y, const int x, const int cheky = 10000, int const chekx = 10000)
	{
		for (int i = 0; i < 4; i++)
		{
			int dy, dx;
	...
	bool Player::ChekDestroy(Game game, const int y, const int x, int** ptable_mode, int** ptable_state, const int cheky = 10000, int const chekx = 10000)
	{
		for (int i = 0; i < 4; i++)
		{
			int dy, dx;
	...
	void Player::explosion(Game game, unsigned int y, unsigned int x)
	{
		for (int i = 0; i < 8; i++)
		{
			int dx, dy;
	...
	void Player::explosion(Game game, unsigned int y, unsigned int x, int** ptable_mode, int** ptable_state)
	{
		for (int i = 0; i < 8; i++)
		{
			int dx, dy;
	...
	Ships::Ships(Game game, Player player, int deck)
	{
		deck_ = deck;
		SetShipAuto(game, player, deck_);
	}
	int Ships::chekCoord(Game & game, Player player, int y, int x)
	{
	...
	void Ships::SetShipAuto(Game & game, Player player, int deck_)
	{
